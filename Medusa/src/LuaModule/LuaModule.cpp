...
//nitskill:
extern "C" int Skill_GoalieV1(lua_State *L)
{
    TaskT playerTask;
    int runner = int(LuaModule::Instance()->GetNumberArgument(1, 0));
    playerTask.executor = runner;
    CPlayerTask* pTask = TaskFactoryV2::Instance()->GoalieV1(playerTask);
    TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);

    return 0;
}

extern "C" int Skill_GoRush(lua_State *L)
{
    TaskT playerTask;
    int runner = int(LuaModule::Instance()->GetNumberArgument(1, 0));
    playerTask.executor = runner;
    CPlayerTask* pTask = TaskFactoryV2::Instance()->GoRush(playerTask);
    TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
...
extern "C" int Skill_Getball(lua_State *L)
{
    ...
    double y = LuaModule::Instance()->GetNumberArgument(3, 0);
    playerTask.player.pos = CGeoPoint(x,y);
    CPlayerTask* pTask = TaskFactoryV2::Instance()->Getball(playerTask);
    TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
...
luaDef GUIGlue[] = 
{
    ...
    {"CGoalieV1",           Skill_GoalieV1},
    {"CGoRush",             Skill_GoRush},
